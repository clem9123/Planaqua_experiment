...
library(writexl)
library(truncnorm)
setwd("C:/Users/sandrine/Documents/IEES_stage/code_et_donnee/data/")
BDD <- read_excel("BDD_original_complet.xlsx", sheet = "BDD", col_types = "text")
```
...
```{r}
write_xlsx(BDD_f,"C:/Users/sandrine/Documents/IEES_stage/code_et_donnee/data/data_final.xlsx")
```
...
```{r}
BDD_f <- read_excel("data_final.xlsx", col_types = c("numeric","date","text","text","text","text","text","text","text","numeric","numeric","text","text"), na = "")
...
for (i in 1:nrow(BDD_juv)){
  ...
  if (is.numeric(subset(juv_ws, Year== y & Lake_capture == r$Lake_capture)$mean_weight)){
    BDD_f [r$Index-1,]$Weight <- rtruncnorm(1, a= 0.1, b= 200, mean = subset(juv_ws, Year== y & Lake_capture == r$Lake_capture)$mean_weight,
                                          sd = subset(juv_ws, Year== y & Lake_capture == r$Lake_capture)$sd_weight, mean )
  }
  if (is.numeric(subset(juv_ws, Year== y & Lake_capture == r$Lake_capture)$mean_size)){
    BDD_f [r$Index-1,]$Size <- rtrucnorm (1, a= 0.1, b= 200, mean = subset(juv_ws, Year== y & Lake_capture == r$Lake_capture)$mean_size,
...
BDD_vid <- BDD_f %>% filter(!is.na(Tag_id) & Tag_id != "juvenile" & 
                              Tag_id %in% subset(BDD_f, Year == "2021" & Method_capture == "Chalut")$Tag_id &
                              (is.na(Weight) | is.na(Size)) &
                              Year == "2022")

for (i in 1: nrow(BDD_vid)){
  r = BDD_vid[i,]
  BDD_f[r$Index-1,]$Weight <- BDD_f[which(BDD_f$Tag_id == r$Tag_id &
                                            BDD_f$Year == "2021" &
                                            BDD_f$Method_capture == "Chalut"),]$Weight
  BDD_f[r$Index-1,]$Size <- BDD_f[which(BDD_f$Tag_id == r$Tag_id &
                                          BDD_f$Year == "2021" &
                                          BDD_f$Method_capture == "Chalut"),]$Size
}

BDD_f <- BDD_f %>% mutate(Date = as.Date(BDD_f$Date))
BDD_f <- BDD_f[,1:13]
write_xlsx(BDD_f, "C:/Users/sandrine/Documents/IEES_stage/code_et_donnee/data/data_final_ws_norm.xlsx")
```
...
BDD_f <- BDD_f %>% mutate(Date = as.Date(BDD_f$Date))
BDD_f <- BDD_f[,1:13]
write_xlsx(BDD_f, "C:/Users/sandrine/Documents/IEES_stage/code_et_donnee/data/data_final_ws.xlsx")
