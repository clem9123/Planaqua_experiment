...
s <- BDD_p %>% filter(Year != "2022" & !(Tag_id %in% c("no_tag","juvenile"))) %>% ungroup () %>% pivot_wider(id_cols = Tag_id, names_from = Year, values_from = Size)

tr <- BDD_p %>% filter(Year != "2022") %>% group_by(Tag_id, Treatment) %>% summarize() %>% ungroup() #%>% filter(Year != "2022") 
...
la <- BDD_p %>% group_by(Tag_id, Lake) %>% summarize() %>% ungroup()
...
Perch_model_Treatment_capture <- jags.parallel(data = jags.data,
                                            inits = inits,
...
+3
save(Perch_model_Treatment_capture, file = "R/model_final/Perch_model_Treatment_capture.RData" )
