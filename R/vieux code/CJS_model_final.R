...
multievent_treatment_random_corrected_abundance <- function(){
  ...
  for (t in 1:(noccas-1)){
    for (l in 1:16){
      for (gs in 1:3){
        epsilon[l,t,gs] ~ dnorm(0,1/(sigma^2))
      }
      for (gs in 1:2){
        mu[l,t,gs] ~ dnorm(0,1/(tau^2))
      }
    }
  }
  ...
  for (t in 1:2){
    for (l in 1:16){
      logit(phi1_aux[l,t]) <- logit(phi1[LT[l],1]) + epsilon[l,t,1]
      logit(phi2_aux[l,t]) <- logit(phi2[LT[l],1]) + epsilon[l,t,2]
      logit(phi3_aux[l,t]) <- logit(phi3[LT[l],1]) + epsilon[l,t,3]
      
      logit(psi12_aux[l,t]) <- logit(psi12[LT[l],1]) + mu[l,t,1]
      logit(psi23_aux[l,t]) <- logit(psi23[LT[l],1]) + mu[l,t,2]
      #psi13_aux[l,t] ~ dnorm(psi13[tr,1], 1/(sigma^2))
    }
  }
  ...
  for (t in 3:(noccas-1)){
    for (l in 1:16){
      logit(phi1_aux[l,t]) <- logit(phi1[LT[l],2]) + epsilon[l,t,1]
      logit(phi2_aux[l,t]) <- logit(phi2[LT[l],2]) + epsilon[l,t,2]
      logit(phi3_aux[l,t]) <- logit(phi3[LT[l],2]) + epsilon[l,t,3]
      
      logit(psi12_aux[l,t]) <- logit(psi12[LT[l],2]) + mu[l,t,1]
      logit(psi23_aux[l,t]) <- logit(psi23[LT[l],2]) + mu[l,t,2]
      #psi13_aux[l,t] ~ dnorm(psi13[tr,2], 1/(sigma^2))
    }
  }
...
inits = function(){
  list(phi1 = matrix(ncol = 2, runif(8,0,1)),phi2 = matrix(ncol = 2, runif(8,0,1)),phi3 = matrix(ncol = 2, runif(8,0,1)),
       p1 = runif(1,0,1),p2 = runif(1,0,1),p3 = runif(1,0,1),
       psi12 = matrix(ncol = 2, runif(8,0,0.5)),psi23 = matrix(ncol = 2, runif(8,0,0.5)), #psi13 = matrix(ncol = 2, runif(8,0,0.5)),
       error = runif(1,0,0.1), sigma = runif(1,0,1),
       epsilon = array(runif(240,0,1),c(16,5,3)),mu = array(runif(160,0,1),c(16,5,2)),
       z = zi)}

parameters = c("phi1","phi2","phi3",
               "p1","p2","p3",
               "psi12","psi23",
               "epsilon","mu","sigma","tau",
               "error", 
               "n1","n2","n3","ntot",
               "phi1_aux","phi2_aux","phi3_aux",
               "psi12_aux","psi13_aux","psi23_aux")
...
Model_multi_treatment_random_corrected_abundance3 <- jags.parallel(data = jags.data,
                                                                   ...
                                                                   n.chains = 2,
                                                                   n.iter = ni)
save(Model_multi_treatment_random_corrected_abundance3, file = "R/object/Model_multi_treatment_random_corrected_abundance3.RData" )
...
#-----------------------------------------------------------------------------------------------
################################################################################################
# Model corrected with abundance effet random lake et time 4
################################################################################################
#-----------------------------------------------------------------------------------------------
...
multievent_treatment_random_corrected_abundance <- function(){
  ...
  for (t in 1:2){
    for (l in 1:16){
      ...
      logit(psi12_aux[l,t]) <- logit(psi12[LT[l],1]) + mu[l,t]
      logit(psi23_aux[l,t]) <- logit(psi23[LT[l],1]) + mu[l,t]
      #psi13_aux[l,t] ~ dnorm(psi13[tr,1], 1/(sigma^2))
    }
  }
  ...
  for (t in 3:(noccas-1)){
    for (l in 1:16){
      ...
      logit(psi12_aux[l,t]) <- logit(psi12[LT[l],2]) + mu[l,t]
      logit(psi23_aux[l,t]) <- logit(psi23[LT[l],2]) + mu[l,t]
      #psi13_aux[l,t] ~ dnorm(psi13[tr,2], 1/(sigma^2))
    }
  }
...
inits = function(){
  list(phi1 = matrix(ncol = 2, runif(8,0,1)),phi2 = matrix(ncol = 2, runif(8,0,1)),phi3 = matrix(ncol = 2, runif(8,0,1)),
       ...
       psi12 = matrix(ncol = 2, runif(8,0,0.5)),psi23 = matrix(ncol = 2, runif(8,0,0.5)), #psi13 = matrix(ncol = 2, runif(8,0,0.5)),
       error = runif(1,0,0.1), sigma = runif(1,0,1),
       epsilon = array(runif(80,0,1),c(16,5)),mu = array(runif(80,0,1),c(16,5)),
       z = zi)}
...
Model_multi_treatment_random_corrected_abundance4 <- jags.parallel(data = jags.data,
                                                                   inits = inits,
                                                                   parameters.to.save = parameters,
                                                                   model.file = multievent_treatment_random_corrected_abundance,
                                                                   n.chains = 2,
                                                                   n.iter = ni)
save(Model_multi_treatment_random_corrected_abundance4, file = "R/object/Model_multi_treatment_random_corrected_abundance4.RData" )
