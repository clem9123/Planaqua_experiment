...
t[is.na(t)] <- 0
t <- cbind(t,Lake) %>% merge(Lake_treatment, all = TRUE)
capt_hist <- data.frame(ch = unite(s_group, 
                        col = "ch", 
                        sep = ""),
                        Lake = s$Lake,
                        Treatment = s$Treatment)

capt_hist.proc <- process.data(capt_hist, model = "Multistrata", groups = c("Lake","Treatment"))
capt_hist.ddl <- make.design.data(data = capt_hist.proc)
```
...
phi.p.psi <- mark(capt_hist.proc,
                 capt_hist.ddl,
                 model.parameters = list(S = list(formula =~ stratum*Treatment),
