...
Model_Treatment_time_and_lake <- jags.parallel(data = jags.data,
                                               ...
                                               n.chains = 2,
                                               n.iter = ni)
save(Model_Treatment_time_and_lake, file = "R/model_cmr_perch/Model_Treatment_time_and_lake.RData" )
...
Model_Treatment_time_x_lake <- jags.parallel(data = jags.data,
                                              ...
                                              n.chains = 2,
                                              n.iter = ni)
save(Model_Treatment_time_x_lake, file = "R/model_cmr_perch/Model_Treatment_time_x_lake.RData" )
...
#-----------------------------------------------------------------------------------------------
################################################################################################
# Model Treatment + proba_capt pre-growth
################################################################################################
#-----------------------------------------------------------------------------------------------
...
model <- function(){
  ...
  for (t in 1:(noccas-1)){
    for (tr in 1:4) {
      gamma[1,1,tr,t] <- phi1_aux[tr,t] * (1-psi12_aux[tr,t]-psi13_aux[tr,t])
      gamma[1,2,tr,t] <- phi2_aux[tr,t] * psi12_aux[tr,t]
      gamma[1,3,tr,t] <- phi1_aux[tr,t] * psi13_aux[tr,t]
      gamma[1,4,tr,t] <- (1-phi1_aux[tr,t] * (1-psi12_aux[tr,t])-phi2_aux[tr,t] * psi12_aux[tr,t])
      gamma[2,1,tr,t] <- 0
      gamma[2,2,tr,t] <- phi2_aux[tr,t] * (1-psi23_aux[tr,t])
      gamma[2,3,tr,t] <- phi3_aux[tr,t] * psi23_aux[tr,t]
      gamma[2,4,tr,t] <- (1-phi2_aux[tr,t] * (1-psi23_aux[tr,t])-phi3_aux[tr,t] * psi23_aux[tr,t])
      gamma[3,1,tr,t] <- 0
      gamma[3,2,tr,t] <- 0
      gamma[3,3,tr,t] <- phi3_aux[tr,t]
      gamma[3,4,tr,t] <- (1-phi3_aux[tr,t])
...
Model_treatment_capture_reverse <- jags.parallel(data = jags.data,
                                            inits = inits,
                                            parameters.to.save = parameters,
                                            model.file = model,
                                            n.chains = 2,
...
save(Model_Treatment_capture_reverse, file = "R/model_cmr_perch/Model_Treatment_capture_reverse.RData" )
