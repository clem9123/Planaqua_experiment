...
BDD_vid <- BDD_vid %>% mutate (catch_2021 = case_when(Year == "2021" ~ TRUE,
                                      Tag_id %in% subset(BDD_vid, Year == "2021")$Tag_id ~ TRUE,
                                      TRUE ~ FALSE),
...
```{r}
BDD_vid_n <- BDD_vid %>% group_by(taille,Lake,Year) %>% summarise(nb_catch_recatch = sum(catch_2021), n_nouv = sum(!catch_2021))
```
```{r}
BDD_vid <- BDD_vid %>% mutate(taille = case_when(Tag_id == "juvenile" ~ "juv",
                                         Weight < 20 ~ "0-20",
...
ggplot()+
  ...
  geom_point(aes(y=invlogit(0.07*x-0.00025*x^2-2), x = x), color = "blue")+
  geom_point(aes(y=invlogit(0.05*x-0.0004*x^2-3), x = x), color = "red")+
  geom_point(aes(y=invlogit(0.08*x-0.0004*x^2+2), x = x), color = "red")
...
```{r}
BDD_vid_survie <- BDD_vid %>% filter(catch_2021) %>% group_by(taille,  Year) %>% summarise(nb = n())
...
t <- BDD_vid %>% filter(Year == "2021") %>% mutate(survie = as.numeric(survie),
                                                   Size_2 = Size^2)
model_phi <- glmer(survie ~ Size + Size_2 + (1|Lake), family = "binomial", t)
summary(model_phi)
```
...
ggplot()+
  ...
  geom_point(aes(y=invlogit(0.11*x-0.00025*x^2-7), x = x), color = "red")+
  geom_point(aes(y=invlogit(0.02*x+5), x = x), color = "blue")+
  geom_point(aes(y=invlogit(0.04*x), x = x), color = "green")+
  geom_point(aes(y=invlogit(0.11*x+5), x = x), color = "violet")
```
```{r}
summary(model_phi2)
```
...
C:/Users/sandrine/Documents/IEES_stage/code_et_donnee/Etude_vidange.Rmd