...
BDD_vid <- BDD_vid %>% mutate(s_group = discretize(BDD_vid$Size,method = "fixed",
                           breaks = size_break, 
...
t <- BDD_vid %>% filter(Year == "2022") %>% mutate(catch_2021 = as.numeric(catch_2021),
                                                   Size = 10^-2*Size,
                                                   Size_2 = Size^2)
model_pc <- glmer(catch_2021 ~ Size + Size_2 + density + (1|Lake), family = "binomial", t)
summary(model_pc)
```
...
ggplot(BDD_vid %>% filter (Year == "2022") %>% group_by(Lake, Treatment, s_group) %>% summarize(density = n()))+
  geom_col(stat = "identity",position = "fill",aes (y = density,  x = Lake, fill = s_group))+
