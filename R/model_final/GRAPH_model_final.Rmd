...
```{r}
model = as.mcmc(Model_Treatment_p1)
p = data.frame()
for (n in 1:2){
    for (gs in 1:3){
        p <- rbind(p, 
...
print(Model_Treatment_p1[[2]]$DIC)
...
for (n in 1:2){
    ...
    deviance <- rbind (deviance, dev = data.frame(as.mcmc(Model_Treatment_p1)[[n]][,"deviance"],name = "Model_Treatment_p1"))
    deviance <- rbind (deviance, dev = data.frame(as.mcmc(Model_treatment_capture_p1)[[n]][,"deviance"],name = "Model_treatment_capture_p1"))
}


sum_deviance = deviance %>% group_by(name) %>% summarize(mean = mean(var1), min = quantile(var1,0.025), max = quantile(var1,0.975)) %>% mutate(name = factor(name, level = c("Model_Treatment","Model_Treatment_time_x_lake","Model_Treatment_time_and_lake","Model_Treatment_capture","Model_Treatment_p1","Model_treatment_capture_p1")))
...
```{r}
traceplot(Model_treatment_capture_p1, ask = FALSE)
```
