...
```{r}
t <- BDD_g %>% filter (Year == "2022" & !Tag_id%in% c("juvenile","no_tag")) %>% group_by(Treatment,Lake, s_group) %>% summarize(nb = n())
...
```{r}
t <- BDD_g %>% filter(Year == "2022" & !Tag_id %in% c("juvenile","no_tag")) %>% 
               mutate (catch_2021 = case_when(Year == "2021" ~ TRUE,
...
p1 <- ggplot(s %>% filter (catch + new > 10))+
  ...
  labs(y = "Observed capture probability")+
...
```{r}
t <- BDD_g %>% filter(Year == "2021" & !Tag_id %in% c("juvenile","no_tag") & s_group %in% c(1,2,3)) %>% mutate(survie = case_when(Year == "2022" ~ TRUE,
                                      Tag_id %in% subset(BDD_g, Year == "2022")$Tag_id ~ TRUE,
                                      TRUE ~ FALSE)) %>%
                  mutate(survie = as.numeric(survie))
t$index <-  c(1:nrow(t))
model_phi <- glmer(survie ~ s_group*Treatment +(1|Lake), t, family = "binomial", control = glmerControl(optimizer = "Nelder_Mead"))
summary(model_phi)
```
...
model_phi <- glmer(survie ~ s_group +(1|Lake), t, family = "binomial")
summary(model_phi)
```
```{r}
m2 = update(model_phi, ~.- s_group)
anova(model_phi, m2)
```
