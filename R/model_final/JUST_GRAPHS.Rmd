...
p1 <- ggplot(BDD_g %>% filter(!is.na(s_group) & Treatment ==1))+
  ...
  labs(title = "Treatment 1", y = "Roach abundance")
...
p2 <- ggplot(BDD_g %>% filter(!is.na(s_group) & Treatment ==2))+
  ...
  facet_wrap(~factor(Lake,levels = c(1,8,3,6,11,14,9,16,2,7,4,5,12,13,10,15),
                        labels = c("Lake 1", "Lake 8", "Lake 3", "Lake 6",
                                   ...
                                   "Lake 2", "Lake 7", "Lake 4", "Lake 5",
                                   "Lake 12", "Lake 13", "Lake 10", "Lake 15")))+
  labs(title = "Treatment 2", y = "Roach abundance")
...
p3 <- ggplot(BDD_g %>% filter(!is.na(s_group) & Treatment ==3))+
  ...
  facet_wrap(~factor(Lake,levels = c(1,8,3,6,11,14,9,16,2,7,4,5,12,13,10,15),
                        labels = c("Lake 1", "Lake 8", "Lake 3", "Lake 6",
                                   ...
                                   "Lake 2", "Lake 7", "Lake 4", "Lake 5",
                                   "Lake 12", "Lake 13", "Lake 10", "Lake 15")))+
  labs(title = "Treatment 3", y = "Roach abundance")
...
p4 <- ggplot(BDD_g %>% filter(!is.na(s_group) & Treatment ==4))+
  ...
  facet_wrap(~factor(Lake,levels = c(1,8,3,6,11,14,9,16,2,7,4,5,12,13,10,15),
                        labels = c("Lake 1", "Lake 8", "Lake 3", "Lake 6",
                                   ...
                                   "Lake 2", "Lake 7", "Lake 4", "Lake 5",
                                   "Lake 12", "Lake 13", "Lake 10", "Lake 15")))+
  labs(title = "Treatment 4", y = "Roach abundance")
...
m1 <- glmer(nb ~ s_group*Treatment + (1|Lake), family = "poisson", t)
summary(m1)
```
...
```{r}
t <- BDD_vid %>% filter(Year == "2021") %>% mutate(survie = case_when(Year == "2022" ~ TRUE,
...
c <- c %>% mutate(sg_2021 = ifelse(is.na(sg_2021),"Not catched", sg_2021),
             sg_2022 = ifelse(is.na(sg_2022),"Dead", sg_2022)) %>%
  mutate(sg_2022 = factor (sg_2022), sg_2021 = factor (sg_2021))


ggplot(c ,aes(axis1 = factor(sg_2021, levels = c("4","1","2","3"), labels = c("Not_catched","Small","Medium","Large")),
              axis2 = factor(sg_2022, levels = c("4","1","2","3"), labels = c("Dead","Small","Medium","Large"))))+
  geom_alluvium(aes(fill = sg_2022),width = 1/12) +
  scale_fill_brewer(palette = "YlOrBr","Size group in 2022", labels = c("Dead","Small","Medium","Large"))+
