...
for (i in 1:6){
  for (t in 1:6){
    for (l in 1:16){
      tbl_p <- rbind(tbl_p, 
               data.frame(as.mcmc(CJS_group_taille_tr_p)[[1]][,paste("p[",as.character(i),",",as.character(t),",",as.character(l),"]", sep = "")],i,t,l))
    }
  }
  
  for (j in 1:4){
    tbl_phi <- rbind(tbl_phi, 
               data.frame(as.mcmc(CJS_group_taille_tr_p)[[1]][,paste("phi[",as.character(i),",",as.character(j),"]", sep = "")],i,j))
  }
}
colnames(tbl_phi) <- c("phi","taille","treatment")
tbl_phi <- tbl_phi %>% mutate(taille = factor (taille),
