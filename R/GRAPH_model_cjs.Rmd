...
```{r}
model = Model_multi_treatment_random_corrected_abundance4 #Model_multi_treatment_corrected_abundance
...
for (n in 1:2){
  for (tr in 1:4){
    for (t in 1:2){
      tbl5_psi12 <- rbind(tbl5_psi12, 
               data.frame(as.mcmc(model)[[n]][,paste("psi12","[",as.character(tr),",",as.character(t),"]", sep = "")],tr,t))
      tbl5_psi23 <- rbind(tbl5_psi23, 
               data.frame(as.mcmc(model)[[n]][,paste("psi23","[",as.character(tr),",",as.character(t),"]", sep = "")],tr,t))
      
      for (gs in 1:3){
        tbl5_phi <- rbind(tbl5_phi, 
               data.frame(as.mcmc(model)[[n]][,paste("phi",as.character(gs),"[",as.character(tr),",",as.character(t),"]", sep = "")],gs,tr,t))
      }
    }
    for (t in 1:6){
      tbl5_ntot <- rbind(tbl5_ntot, 
               data.frame(as.mcmc(model)[[n]][,paste("ntot","[",as.character(t),",",as.character(tr),"]", sep = "")],"tot",tr,t))
      for (gs in 1:3){
        tbl5_n <- rbind(tbl5_n, 
               data.frame(as.mcmc(model)[[n]][,paste("n",as.character(gs),"[",as.character(t),",",as.character(tr),"]", sep = "")],gs,tr,t))
      }
    }
  }
  for (gs in 1:3){
    tbl5_p <- rbind(tbl5_p, 
               data.frame(as.mcmc(model)[[n]][,paste("p",as.character(gs), sep = "")],gs))
  }
}
...
Mes conclusions : Ajout des perches : probabilité de croissance plus importante de 1 a 2
Et sans perche diff probabilité de passer de 2 à 3 augmenté par la présence de nutrients
...
```{r}
print(Model_multi_treatment_random_corrected_abundance[[2]]$mean$phi1)
...
for (l in 1:16){
  for (t in 1:5){
    tbl1 <- rbind(tbl1,data.frame(Model_multi_treatment_random_corrected_abundance[[2]]$mean$phi1_aux[l,t],as.character(l),as.character(t)))
    tbl2 <- rbind(tbl2,data.frame(Model_multi_treatment_random_corrected_abundance[[2]]$mean$phi2_aux[l,t],as.character(l),as.character(t)))
    tbl3 <- rbind(tbl3,data.frame(Model_multi_treatment_random_corrected_abundance[[2]]$mean$phi3_aux[l,t],as.character(l),as.character(t)))
  }
}
...
for (tr in 1:4){
  for (t in 1:2){
    tbl1mean <- rbind(tbl1mean,data.frame(Model_multi_treatment_random_corrected_abundance[[2]]$mean$phi1[tr,t],as.character(tr),as.character(t)))
    tbl2mean <- rbind(tbl2mean,data.frame(Model_multi_treatment_random_corrected_abundance[[2]]$mean$phi2[tr,t],as.character(tr),as.character(t)))
    tbl3mean <- rbind(tbl3mean,data.frame(Model_multi_treatment_random_corrected_abundance[[2]]$mean$phi3[tr,t],as.character(tr),as.character(t)))
  }
}


colnames(tbl1) <- c("phi","Lake","year")
tbl1 <- merge(tbl1,Lake_treatment)

colnames(tbl2) <- c("phi","Lake","year")
tbl2 <- merge(tbl2,Lake_treatment)

colnames(tbl3) <- c("phi","Lake","year")
tbl3 <- merge(tbl3,Lake_treatment)

colnames(tbl1mean) <- c("phi","Treatment","year")
colnames(tbl2mean) <- c("phi","Treatment","year")
colnames(tbl3mean) <- c("phi","Treatment","year")


p1 = ggplot(tbl1)+
  geom_jitter(aes(x = year, y = phi, color = Treatment), width = 0.2)+
  ylim(0,1)
p2 = ggplot(tbl1mean)+
  geom_point (aes(x = year,y = phi,color = Treatment), shape = "square", size = 5)+
  ylim(0,1)
p1+p2
...
```
```{r}
traceplot(Model_multi_treatment_random_corrected_abundance, varname = c("phi1_aux"), ask = F)
```
...
for (n in 1:2){
  for (tr in 1:4){
    for (t in 1:2){
      tbl5_psi12 <- rbind(tbl5_psi12, 
               data.frame(as.mcmc(Model_multi_treatment_random_corrected_abundance)[[n]][,paste("psi12","[",as.character(tr),",",as.character(t),"]", sep = "")],tr,t))
      tbl5_psi23 <- rbind(tbl5_psi23, 
               data.frame(as.mcmc(Model_multi_treatment_random_corrected_abundance)[[n]][,paste("psi23","[",as.character(tr),",",as.character(t),"]", sep = "")],tr,t))
      
      for (gs in 1:3){
        tbl5_phi <- rbind(tbl5_phi, 
               data.frame(as.mcmc(Model_multi_treatment_random_corrected_abundance)[[n]][,paste("phi",as.character(gs),"[",as.character(tr),",",as.character(t),"]", sep = "")],gs,tr,t))
      }
    }
    for (t in 1:6){
      tbl5_ntot <- rbind(tbl5_ntot, 
               data.frame(as.mcmc(Model_multi_treatment_random_corrected_abundance)[[n]][,paste("ntot","[",as.character(t),",",as.character(tr),"]", sep = "")],"tot",tr,t))
      for (gs in 1:3){
        tbl5_n <- rbind(tbl5_n, 
               data.frame(as.mcmc(Model_multi_treatment_random_corrected_abundance)[[n]][,paste("n",as.character(gs),"[",as.character(t),",",as.character(tr),"]", sep = "")],gs,tr,t))
      }
    }
  }
  for (gs in 1:3){
    tbl5_p <- rbind(tbl5_p, 
               data.frame(as.mcmc(Model_multi_treatment_random_corrected_abundance)[[n]][,paste("p",as.character(gs), sep = "")],gs))
  }
}
